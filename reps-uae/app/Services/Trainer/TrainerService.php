...
//        $id = $this->createId($user->id);
        ...
        $insert_data = array(
            ...
            'mobile_phone' => isset($userDetails['mobile_phone']) ? $userDetails['mobile_phone'] : '',
            'photo' => $photo,
            'image' => $image,
           // 'emirates_id_no' => (isset($userDetails['emirates_id'])?$userDetails['emirates_id']:''),
            'emirates_id_no' => isset($userDetails['emirates_id_no']) ? $userDetails['emirates_id_no'] : '',
            'membership' => isset($userDetails['membership']) ? $userDetails['mobile_phone'] : '',
            'expiry_date' => date('Y-m-d',strtotime('+1 years')),
            'status_id'=>$userDetails['status_id'],
            'work_email' => (isset($userDetails['work_email'])) ? $userDetails['work_email'] : $userDetails['email'],
...
//        $trainer = $user->addTrainer($data);
       ...
       //for image from tmp directory
        File::copyDirectory( Config::get('tmp.tmpPath').'/'.$userDetails['tmpFolder'].'/image',
    ...
    public function saveTrainerWorkPlace($trainer,$user)
    {
        ...
        if(empty($userDetails)){
            $userDetails = Trainer::getTempTrainerByEmail($trainer->work_email);
            $userDetails = (array) json_decode($userDetails->json_data);
        }
...
//        ));
    ...
    public function saveTrainerQualifications($trainer, $user)
    {
        ...
        if (!empty($userDetails['aid'][0]['name'])) {
            $aid = $userDetails['aid'][0]['name'];
        } else {
            $aid = '';
        }
...
//            ));
        ...
        //make the aid directory
        File::makeDirectory(Config::get('trainer.path').'/'.$user->id.'/aid', 0755, true);
        
        if ($aid) {
            //mv the aid directory form the tmp directory
            File::copyDirectory( Config::get('tmp.tmpPath').'/'.$userDetails['tmpFolder'].'/aid',
                Config::get('trainer.path').'/'.$user->id. '/aid');
        }
    ...
    */
    ...
    public function validateWorkExperience($data)
    {
        ...
        {
            $results['messageBag']->add('cv', 'The cv is required.');
				return array('status'=>true,'messageBag'=>$results['messageBag']);
        }
              ...
              $file_loc1 = Config::get('trainer.path').'/'.$user->id.'/image/'.$data['image'];
                ...
                if ($img->height() >= $img->width()) {
                    $img->resize(null, 170, function ($constraint) {
                        ...
                        ->resizeCanvas(130, 170, 'center', false, '#ffffff')
                        ->save($file_loc1, 100);
                } else {
