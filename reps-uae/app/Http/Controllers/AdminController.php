...
class AdminController extends BaseController
{
    ...
    public function authenticate()
    {
        ...
        try {
            ...
            // Get the class name of the exception to determine the type
            ...
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Users\PasswordRequiredException') {
                $message['text'] = 'Password is required';
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Users\WrongPasswordException') {
                $message['text'] = 'Invalid user credentials';
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Users\UserNotActivatedException') {
                $message['text'] = 'User is not active';
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Users\UserNotFoundException') {
                $message['text'] = 'Invalid user credentials';
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Throttling\UserSuspendedException') {
                try {
                    $throttle = Sentry::findThrottlerByUserId(request()->get('email'));
                    $time = $throttle->getSuspensionTime();
                    $message['text'] = "User is suspended for [$time] minutes";
                } catch (\Exception $innerEx) {
                    $message['text'] = "User is suspended";
                }
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Throttling\UserBannedException') {
                $message['text'] = "User is banned.";
            }

            return Redirect::action('AdminController@logIn')->with('message', $message);
        }
    }
    ...
    public function save()
    {
        ...
        switch (request()->get('form')) {
            ...
            case 'Personal Details':
                ...
                $data['e_certificate'] = request()->file('e_certificate');
            ...
            case 'Work Experience':
                ...
                if (
                    ...
                    $userData['emirates_id_no'] = $userData['emirates_id_no'];
                    $userData['membership'] = $userData['membership'];
                    // dd($userData['membership']);
                    //save the data in the session
            ...
            case 'Qualifications':
                ...
                $certificates = array(
                    ...
                    request()->file('certificates5'),
                    request()->file('certificates6'),
                    request()->file('certificates7'),
                    request()->file('certificates8'),
                    request()->file('certificates9')
                );
