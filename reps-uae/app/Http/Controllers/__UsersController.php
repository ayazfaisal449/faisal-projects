...
class UsersController extends BaseController {
	...
	* add to mailchimp if not
    ...
    public function save()
    {
        ...
        $file_data3 = Input::file('member_photo');
        $file_data1 = Input::file('image');
        $file_data2 = Input::file('e_certificate');
        $form_data = Input::get();
         // dd($file_data2);
        if (!empty($id)) {
            ...
            if ($validator->fails() || $pErrors['status']) {
                ...
                if (Input::hasFile('image')) {
                 ...
                 if (Input::hasFile('e_certificate')) {
                    // dd('here');
                    if (is_file($fn2)) {
                        unlink($fn2);
                    }
...
//            echo $temppath = public_path() . "/tmp/newtemptest/";
