...
class BaseController extends Controller
{
     ...
     * Created By Kevin @ Cranium Creations
     */
    public function paginator($data, $dataPerPage, $appends = [])
    {
        ...
        $currentItems = $dataCollection->slice(($currentPage - 1) * $dataPerPage, $dataPerPage)->values();
    ...
    public function getTmpFiles($files, $user = NULL)
    {
        ...
        // Ensure $files['name'] is an array
        if (!isset($files['name'])) {
            return $default;
        }
    
        $noFiles = is_array($files['name']) ? count($files['name']) : 1;
    
        if ($noFiles > 1 || is_array($files['name'])) {
            for ($i = 0; $i < $noFiles; $i++) {
                $ext = strstr($files['name'][$i], '.');
                $default[] = array(
                    'name' => $this->getSlug(strstr($files['name'][$i], '.', true)) . $ext,
                    'type' => $files['type'][$i],
                    'size' => $files['size'][$i],
            ...
            $default[] = array(
                'name' => $this->getSlug(strstr($files['name'], '.', true)) . $ext,
                'type' => $files['type'],
                'size' => $files['size'],
    ...
    function createTmpFiles($file, $fileType)
    {
        ...
        if (!isset($userDetails['tmpFolder'])) {
            ...
            //set sub-folders
            File::makeDirectory(
                Config::get('tmp.tmpPath') . '/' . $folder . '/photo',
                0755,
                true
            );

            File::makeDirectory(
                Config::get('tmp.tmpPath') . '/' . $folder . '/image',
                0755,
                true
            );

            File::makeDirectory(
                Config::get('tmp.tmpPath') . '/' . $folder . '/e_certificate',
                0755,
                true
            );
            ...
            //                0755,true);
            File::makeDirectory(
                Config::get('tmp.tmpPath') . '/' . $folder . '/certificate',
                0755,
                true
            );
            File::makeDirectory(
                Config::get('tmp.tmpPath') . '/' . $folder . '/cv',
                0755,
                true
            );

            File::makeDirectory(
                Config::get('tmp.tmpPath') . '/' . $folder . '/aid',
                0755,
                true
            );
        }
        ...
        switch ($fileType) {
            ...
            case 'aid':
                //check if the tmp folder is set 
                if (is_dir(Config::get('tmp.tmpPath') . '/' . $userDetails['tmpFolder'] . '/aid')) {
                    if ($file) {
                        //create tmp file in tmp folder
                        for ($i = 0; $i < count($file); $i++) {
                            $result = move_uploaded_file(
                                $file[$i]['tmpNamePath'],
                                Config::get('tmp.tmpPath') . '/' . $userDetails['tmpFolder'] . '/aid/' . $file[$i]['name']
                            );
                        }
                    } else {
                        $result = true; //user did not upload a CV
