...
/*
    ...
    public function processRenewal() {
        ...
        */
        ...
        Session::put('payment_type', 'renew');
    ...
    public function resetPassword()
    {
        ...
        Mail::send('trainer.mail.changePassword', array('resetcode'=> $resetCode,'firstname' => $user->first_name
...
//                    $checkTempTrainer->delete();
    ...
    public function paymentResponse($response) {
        //Writing Payfort response in file
        $responseData1 = json_encode(Input::all());
        $responseData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
        $responseData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
        $responseData .= PHP_EOL . "Data: " . $responseData1;
        file_put_contents(Config::get('tmp.tmpPath') . '/payfort_response.txt', $responseData, FILE_APPEND | LOCK_EX);
    ...
    public function response_page($head, $message) {
        ...
        return View::make('payfort.response', $data);
    }
...
//                $user->trainer->forceSave();
    ...
    private function notifyTrainerRegistrationSuccess($user, $input) {
        ...
        Mail::send('emails.registrationNotification', $data,
        function($message) use ($user) {
    ...
    private function notifyAdminPayment($user, $input, $renew = false) {
        ...
        if($renew) {
            ...
            Mail::send('emails.paymentNotification', $data,
            function($message) {
...
//        }
    ...
    public function approvePayment($payment_id = '') {
        ...
        if ($payment->processStatus == 0) {
            ...
            Mail::send('emails.paymentApprovalNotification', $data,
            function($message) use ($user) {
        ...
        if(isset($data['user_id']) && $data['trainer_id'])
        {
            ...
            else
            {
                ...
                if (!empty($emailData)) {
                    ...
                    Mail::send('trainer.mail.user_info_update', $emailData, function($message) use($emailData) {
      ...
      //   die();
        ...
        if(isset($data) && !empty($data))
        {
            ...
            else
            {
                ...
                Mail::send('emails.upgradeNotificationLevel', array('name' => $name
    ...
    public function trainerUpgradeStatus()
    {
        ...
        if(isset($data) && !empty($data))
        {
            ...
            else
            {
                ...
                Mail::send('emails.upgradeNotificationStatus', array('name' => $name
                    ), function($message) use($name) {
                        $message->to(Config::get('contact.emailContactFrom'),'Admin')
