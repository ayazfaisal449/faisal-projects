...
class TrainerController extends BaseController
{
    ...
    public function processRenewal(Request $request)
    {
        ...
        try {
            ...
            if (isset($user->trainer->status_id) && $user->trainer->status_id == 1) {
            ...
            if (!empty($request->file('photo'))) {
                foreach ($request->file('photo') as $test) {
        ...
        */
            ...
            // dd($ab );
            // new code added started
            $concatenatedIds = DBS::table('trainer_registration_category')
                ->select(DBS::raw('GROUP_CONCAT(registration_category_id) as concatenated_ids'))
                ->where('trainer_id', $user->trainer->id)
                ->whereNull('deleted_at') // Assuming deleted_at is a nullable timestamp field
                ->groupBy('registration_category_id') // Assuming you want to group by trainer_id
                ->orderBy('id', 'desc')
                ->get();
            $categoriesArray = array();
            foreach ($concatenatedIds as $item) {
                $categoriesArray[] = $item->concatenated_ids;
            }
            // new code added end

            if ($this->trainerIsExpired($user->trainer->expiry_date)) {

                if (array_intersect([6, 7, 8, 4], $categoriesArray)) {
                    $vatRatePercentage = 5;
                    $netPrice = 450;
                    $penalty = 100;
                    $totalprice = $penalty + $netPrice;
                    $vatRate = $vatRatePercentage / 100;
                    $vatAmount = $totalprice * $vatRate;
                    $amount = $totalprice + $vatAmount;
                } else {
            ...
            $payfort_data = array(
                ...
                'SHASIGN' => $shasign,
                'TYPE' => 'renew',
                'FILES' => implode(",", $files_list),
                'TRAINER_ID' => $user->trainer->id,
            );
            // dd('here');

            $data1['payment_type'] = 'renew';
            ...
            $efile = $e->getFile();
            $eline = $e->getLine();
            $writeData .= PHP_EOL . " $emsg in file $efile on line $eline";
            file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);
        }
    }
    ...
    public function trainerWorkExperienceForm()
    {
        ...
        $trainerDetails = Session::get('trainerDetails');
        $data = array(
            'email' => $trainerDetails['email'],
            // 'countries' => Country::lists('name', 'id'),
            'regCategory' => Trainer::getRegistrationCategory(),
            'work_place' => isset($errors['work_place']) ? $errors['work_place'] : '',
    ...
    private function validateUserData($data)
    {
        ...
        $validator = Validator::make(
            ...
            array(
                ...
                'password' => 'required|min:8|same:confirm_password', // Use the "same" rule
    ...
    public function save(Request $request)
    {
        ...
        switch ($request->get('form')) {
            ...
            case 'Personal Details':
                ...
                Trainer::updateTempTrainer(array(
                    'email'     => $data['email'],
                    'json_data' => json_encode($trainerDetails)
                ));
                // $this->createTmpFiles($data['e_certificate'],'e_certificate');

                if ($result) {

                    return Redirect::to('/trainer/trainerWorkExperienceForm');
                } else {
            ...
            case 'Work Experience':
                ...
                if (
                    ...
                    $userData = Trainer::getTempTrainerByEmail($data['email']);
                    $userData = (array) json_decode($userData->json_data);
            ...
            case 'Qualifications':
                ...
                $userData = Trainer::getTempTrainerByEmail($post_data['email']);
                $userData = (array) json_decode($userData->json_data);
                if (isset($post_data['avail_insurance'])) {
                    ...
                    $userData['insurance_data'] = $insurance_data;
                }
                ...
                $courseProvider = $request->get('course_provider');
                $dateCompleted = $request->get('date_completed');
                $data = array();

                //get all the certificates submitted laravel object
                $certificates = array(
                    $request->file('certificates0'),
                    $request->file('certificates1'),
                    $request->file('certificates2'),
                    $request->file('certificates3'),
                    $request->file('certificates4')
                ...
                if (empty($trainer)) {
                    Log::channel('no_trainer')->info('No trainer data' . date('Y-m-d H:i:s'));
                ...
                //                ));
                $aid = $this->getTmpFiles($_FILES['aid']);
                $aidFile = $this->createTmpFiles($aid, 'aid');
                $aidFile = $aid;
                $trainer['aid'] = $aidFile;
            ...
            */
                ...
                //new added
                $data1['checkout_email'] = $trainer['email'];
                $data1['payment_type'] = 'new';

                // $amount = (420 * 100) + ($insurance_fee * 100); // amount 420 for new registration
                $userData = Session::get('trainerDetails');
                $categories = $userData['registration_cateogry_id'];

                // Check if any of the specified category IDs (6, 7, 8, 4) is present in the user's categories
                if (array_intersect([6, 7, 8, 4], $categories)) {
                    $amount = 450 + (450 * 0.05); // Dhs 450 plus 5% VAT

                } else {
                    // You can set the default amount here, for example:
                    $amount = 400 + (400 * 0.05);
                }
                if ($insurance_fee != 0) {
                    $amount = 112875;
    ...
    public function paymentResponse(Request $request,$response)
    {
        ...
        if ($response == 'success') {
            ...
            if ($request->get('SHASIGN')) {

                $input = array(
                    'sid'         => $request->get('SHASIGN'),
                    'total'       => $request->get('amount'),
                    'order_number' => $request->get('orderID'),
                    'key'         => $request->get('SHASIGN'),
                    'email'       => Session::get('checkout_email'),
                    'invoice_id'  => $request->get('PAYID')
                );
                ...
                if ($payment_type == 'new') {
                    ...
                    try {
                        ...
                        $input['order_number'] = $request->get('orderID');
                    ...
                    try {
                        ...
                        if (isset($trainer->users->email)) {
                            ...
                            try {
                                ...
                                date_default_timezone_set('Asia/Dubai');
                                $writeData1 = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                                $writeData1 .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                                $writeData1 .= PHP_EOL . "findUserByLogin";
                                $emsg = $e->getMessage();
                                $efile = $e->getFile();
                                $eline = $e->getLine();
                                $writeData .= PHP_EOL . " $emsg in file $efile on line $eline";
                                file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData1, FILE_APPEND | LOCK_EX);
                            }
                            ...
                            if (empty($user)) {
                                ...
                                $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                                $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                                $writeData .= PHP_EOL . "writeNoUserData: " . date("Y-m-d h:i:s a");
                                $writeData .= PHP_EOL . "UserId: " . $trainer->users->id;
                                file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);
                            } elseif (empty($user->trainer)) {
                                date_default_timezone_set('Asia/Dubai');
                                $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                                $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                                $writeData .= PHP_EOL . "writeNoUserTrainerData: " . date("Y-m-d h:i:s a");
                                $writeData .= PHP_EOL . "UserId: " . $trainer->users->id;
                                file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);
                            }
                        } else {
                            ...
                            $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                            $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                            $writeData .= PHP_EOL . "writeNoEmailData: " . date("Y-m-d h:i:s a");
                            $writeData .= PHP_EOL . "UserId: " . $trainer->users->id;
                            file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);
                        }
    ...
    // payfort response callback
    public function paymentResponse2(Request $request, $response_url)
    {
        if ($request->get('order_description') != 'REPS Membership') {
            $head = 'Invalid Response';
            $message = 'The transaction response from the payment gateway is invalid.';
            return Redirect::action('TrainerController@response_page', [$head, $message]);
        ...
        //Check if they payment is already processed
        $chk = Trainer::getSubscriptionPaymentByInvoiceId($request->get('merchant_reference'));
        if ($chk->count() > 0) {
            if ($request->get('merchant_extra') == 'renew') {
                $head = 'Renewal is on Process';
                $message = 'Your payment has been noted and is awaiting review from a REPs administrator. Please wait 2 to 3 days for processing.';
            } else {
                $head = 'Payment done';
        ...
        $response = $request->all();
        $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
        $writeData .= PHP_EOL . "After payment response: ";
        $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
        $writeData .= PHP_EOL . json_encode($response);
        $writeData .= PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
        file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);

        if ($response_url == 'success' && !empty($response['response_code']) && $response['response_code'] == '14000') {
            // Verify response data
            $sha_out = Config::get('payfort.SHA_IN');
            uksort($response, 'strcasecmp');
            Session::put('customer_email', $request->get('customer_email'));
            $shasign = '';
            foreach ($response as $key => $value) {
                if ($value == '') continue;
                if ($key == 'SHASIGN') {
            ...
            if ($request->get('signature')) {
                ...
                $input = array(
                    'sid'         => $request->get('signature'),
                    'total'       => $request->get('amount'),
                    'order_number' => $request->get('merchant_reference'),
                    'key'         => $request->get('signature'),
                    'email'       => Session::get('customer_email'),
                    'invoice_id'  => $request->get('fort_id')
                );

                $payment_type = ($request->get('merchant_extra') != '') ? $request->get('merchant_extra') : Session::get('payment_type');

                $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                $writeData .= PHP_EOL . "After Payment response check response url: ";
                $writeData .= PHP_EOL . "response_url: " . $response_url;
                $writeData .= PHP_EOL . "Session (customer_email): " . Session::get('customer_email');
                $writeData .= PHP_EOL . "Session (payment_type): " . Session::get('payment_type');
                $writeData .= PHP_EOL . "Input (payment_type): " . $request->get('merchant_extra');
                $writeData .= PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);
                ...
                if ($payment_type == 'new') {
                    ...
                    $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                    $writeData .= PHP_EOL . "New registeration response: ";
                    $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                    $writeData .= PHP_EOL . "Session checkout_email: " . Session::get('customer_email');
                    $writeData .= PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                    file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);

                    try {

                        $input['user_email'] = Session::get('customer_email');

                        $trainerTemp = Trainer::getTempTrainerByEmail(Session::get('customer_email'));
                        if (empty($trainerTemp)) {
                            $msg = "The data corresponding to the registration mail you used is missing.  Please contact REPs UAE (faisal.ayaz@sigmads.com) immediately.";
                            return View::make('trainer.paymenterror')->with('message', $msg);
                        } else {
                        ...
                        $trainer = Session::get('trainerDetails');
                        $user = Sentry::findUserByLogin(Session::get('customer_email'));
                        ...
                        $input['order_number'] = $request->get('merchant_reference');

                        $r = $input;
                        $r['processStatus'] = 1;

                        $this->addPayment($user->trainer, $r);
                        if (Session::get('customer_email') != 'fazal@lbatechnologies.com') {
                            $this->notifyAdminPayment($user, $input);
                            $this->notifyTrainerRegistrationSuccess($user, $input);
                        }

                        if (Session::get('customer_email') != '') {
                            Trainer::deleteTempTrainerByEmail(Session::get('customer_email'));
                        }
                    ...
                    // dd('here');
                    $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                    $writeData .= PHP_EOL . "Renew response: ";
                    $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                    $writeData .= PHP_EOL . "Session checkout_trainer_id: " . Session::get('checkout_trainer_id');
                    $writeData .= PHP_EOL . "Session renewalData: " . json_encode(Session::get('renewalData'));
                    $writeData .= PHP_EOL . "Input checkout_trainer_id: " . $request->get('merchant_extra2');
                    $writeData .= PHP_EOL . "Input renewalData: " . json_encode($request->get('merchant_extra1'));
                    $writeData .= PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                    file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData, FILE_APPEND | LOCK_EX);

                    try {
                        $trainer_id = Session::get('checkout_trainer_id');
                        //If Trainer ID not in session
                        if (empty($trainer_id)) {
                            $trainer_id = $request->get('merchant_extra2');
                        }
                        $trainer = Users::getTrainerById($trainer_id);
                        ...
                        $renewalData = Session::get('renewalData');
                        //If renewal data not in session
                        if (empty($renewalData)) {
                            $renewalDatafiles = explode(",", $request->get('merchant_extra1'));
                            $renewalData['files'] = json_encode($renewalDatafiles);
                        }

                        $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                        $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                        //$writeData .= PHP_EOL . "UserInfo: " . json_decode($trainer->users);
                        $writeData .= PHP_EOL . "UserId: " . $trainer->users->id;
                        ...
                        // $user->trainer->save();
                        $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                        $writeData .= PHP_EOL . "Before payment entry";
                        $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                        $writeData .= PHP_EOL . json_encode($input);
                        $writeData .= PHP_EOL . "UserId: " . $trainer->users->id;

                        $this->addPayment($user->trainer, $input, $certificates);

                        $writeData = PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                        $writeData .= PHP_EOL . "after payment entry";
                        $writeData .= PHP_EOL . "Date: " . date("Y-m-d h:i:s a");
                        $writeData .= PHP_EOL . json_encode($input);
                        $writeData .= PHP_EOL . "UserId: " . $trainer->users->id;
                        $this->notifyAdminPayment($user, $input, true);
        ...
        } else if ($response_url == 'decline') {
            $head = 'Payment declined';
            $message = 'Your transaction has been declined!';
        } else if ($response_url == 'cancel') {
            $head = 'Payment canceled';
            $message = 'Your transaction has been canceled!';
        } else {
            $head = 'Payment error';
    ...
    public function complete_pay(Request $request)
    {
        ...
        $order_num = $request->get('order_number');
        ...
        if (!empty($passback['response_code']) && $passback['response_code'] === $ok_response) {
            ...
            $user_email = $request->get('user_email'); //for registration payment
            $trainer_id = $request->get('trainer_id'); //for trainer subscription renewal
    ...
    public function dashboard()
    {
        ...
        $data = array(
            ...
            'membercard' => $e_certificate[0]->member_photo,
    ...
    public function authenticate(Request $request)
    {
        ...
        try {
            ...
            $user = \Models\Users\Users::where('email', $request->input('email'))->first();
            ...
            if ($exceptionClass == 'Cartalyst\Sentry\Users\LoginRequiredException') {
                return Redirect::back()->withErrors(['Login details are required to proceed further']);
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Users\UserNotActivatedException') {
                return Redirect::back()->withErrors(['User not active or your registration has expired']);
            } elseif ($exceptionClass == 'Cartalyst\Sentry\Users\UserNotFoundException') {
                return Redirect::back()->withErrors(['User not found']);
            }

            return Redirect::back()->withErrors(['An error occurred during authentication']);
        }
    }
    ...
    {
        $userData['work_place'] = $request->input('work_place');
        $userData['cv'] = $request->file('cv');
        $userData['registration_cateogry_id'] = $request->input('registration_category_id');
    ...
    public function updatePersonalDetails(Request $request)
    {
        ...
        if (isset($data['user_id']) && $data['trainer_id']) {
            ...
            if ($errors['status']) {
                ...
                if (isset($data['photo']) && $request->hasFile('image')) {
                    $data['photo'] = $this->getTmpFiles($_FILES['photo']);
                    $data['image'] = $this->getTmpFiles($_FILES['photo']);
                }
     ...
     * Created By Raj @ Cranium Creations
     */
    public function trainerSearchUserIndex(Request $request)
    {
    ...
    public function trainerSearchUsersIndex(Request $request)
    {
        $input = $request->all();
        $term = $request->get('q');
        ...
        if (isset($input) && count($input) > 0) {
            ...
            $db = DBS::table('trainer')
                ->join('trainer_registration_category', 'trainer_registration_category.trainer_id', '=', 'trainer.id')
                ->join('registration_category', 'trainer_registration_category.registration_category_id', '=', 'registration_category.id')
                ->join('status', 'status.id', '=', 'trainer.status_id')
                ->leftJoin('trainer_work_experience', 'trainer_work_experience.trainer_id', '=', 'trainer.id')
                ->join('users', 'users.id', '=', 'trainer.user_id')
                ->join('nationality', 'trainer.nationality_id', '=', 'nationality.id')
                ->where('users.first_name', 'LIKE', '%' . $term . '%')
                ->where('users.activated', '=', 1)
                ->where('trainer.expiry_date', '>=', date('Y-m-d'))
                ->whereNull('trainer_registration_category.deleted_at') // Add this line to filter out NULL values
                ->groupBy('trainer_registration_category.trainer_id')
                ->orderBy('trainer_registration_category.id', 'desc')
                ->get();
     ...
     * Created By Kevin @ Cranium Creations
     */
    public function updateCurrentEmployer(Request $request)
    {
        $data = $request->all();
        $data['cv'] = $request->file('cv');
     ...
     * Created By Kevin @ Cranium Creations
     */
    public function trainerUpgradeLevel(Request $request)
    {
        $data1['setting'] = DBS::table('setting')->get();
        $data = $request->all();
     ...
     * Created By Kevin @ Cranium Creations
     */
    public function trainerUpgradeStatus(Request $request)
    {
        $data1['setting'] = DBS::table('setting')->get();
        $data = $request->all();
    ...
    {
        $data['setting'] = DBS::table('setting')->get();
        $data['registrationCategories'] = DBS::table('registration_category')->select('id', 'level', 'description')->orderby('level')->get();
    ...
    public function temp_trainer(Request $request)
    {
