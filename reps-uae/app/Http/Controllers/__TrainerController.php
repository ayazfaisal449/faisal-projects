...
class TrainerController extends BaseController
{
    ...
    public function resetPassword()
    {
        ...
        Mail::send(
            ...
            array(
                'resetcode' => $resetCode, 'firstname' => $user->first_name
            ),
    ...
    public function save()
    {
        ...
        Log::channel('renew_registration')->info('renew registration');
        ...
        switch (Input::get('form')) {
            ...
            case 'Qualifications':
                ...
                if (empty($trainer)) {
                    Log::useDailyFiles(storage_path() . '/logs/no_trainer.log');
    ...
    // payfort response callback
    public function paymentResponse2($response_url)
    {
        ...
        //Check if they payment is already processed
         $chk = Trainer::getSubscriptionPaymentByInvoiceId(Input::get('merchant_reference'));
         if ($chk->count() > 0) {
            if(Input::get('merchant_extra') == 'renew') {
                $head = 'Renewal is on Process';
                $message = 'Your payment has been noted and is awaiting review from a REPs administrator. Please wait 2 to 3 days for processing.';
            } else {
                $head = 'Payment done';
        ...
        if ($response_url == 'success' && !empty($response['response_code']) && $response['response_code'] == '14000') {
            ...
            $sha_out = Config::get('payfort.SHA_IN');
            uksort($response, 'strcasecmp');
            Session::put('customer_email', Input::get('customer_email'));
            $shasign = '';
            foreach ($response as $key => $value) {
                if ($value == '') continue;
                if ($key == 'SHASIGN') {
            ...
            if (Input::get('signature')) {
                ...
                $payment_type = (Input::get('merchant_extra') != '') ? Input::get('merchant_extra') : Session::get('payment_type');
                ...
                $writeData .= PHP_EOL . "Input (payment_type): " . Input::get('merchant_extra');
                $writeData .= PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData , FILE_APPEND | LOCK_EX);
                ...
                if ($payment_type == 'new') {
                    ...
                    $writeData .= PHP_EOL . "Input checkout_trainer_id: " . Input::get('merchant_extra2');
                    $writeData .= PHP_EOL . "Input renewalData: " . json_encode(Input::get('merchant_extra1'));
                    $writeData .= PHP_EOL . PHP_EOL . PHP_EOL . "----------------";
                    file_put_contents(Config::get('tmp.tmpPath') . '/record.txt', $writeData , FILE_APPEND | LOCK_EX);
                    ...
                    try {
                        ...
                        if(empty($trainer_id)){
                            $trainer_id = Input::get('merchant_extra2');
                        }
                        $trainer = Users::getTrainerById($trainer_id);
                        ...
                        if(empty($renewalData)) {
                            $renewalDatafiles = explode(",", Input::get('merchant_extra1'));
